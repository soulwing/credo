 ...
 * @author Carl Harris
 */
@FacesValidator("org.soulwing.credo.validators.FullName")
    ...
    if (!matcher.matches()) {
      throw new ValidatorException(new FacesMessage(
          FacesMessage.SEVERITY_ERROR, Bundle.getString(
              "fullNameInvalidCharacters", context.getViewRoot().getLocale()), 
          null));
    }
    if (value.length() < MIN_LENGTH || value.length() > MAX_LENGTH) {
      throw new ValidatorException(new FacesMessage(
          FacesMessage.SEVERITY_ERROR, Bundle.getString(
              "fullNameInvalidLength", context.getViewRoot().getLocale()), 
          null));
    }
  }
